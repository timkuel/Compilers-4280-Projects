    ...
    public Token getToken(ArrayList<Character> tempList) {
        ...
        while(!isFinalState) {
            ...
            /* Next state generated an error */
                ...
                System.out.println("\nSCANNER ERROR: " + tables.getTokenHashTable().get(currentState) + " '" + charCheck + "' on line number " + lineNumber + "...Terminating Program");
                System.exit(-1);
            }
            
            /* A '@' was detected, assuming a comment was attempted */
            else if(currentState == -3) {
            	System.out.println("\nSCANNER ERRROR: " + tables.getTokenHashTable().get(currentState) + " detected with the presence of the character '" + tokenBuilder + charCheck + "' on line number " + lineNumber);
